...
def test_manual_direction_review_message_handles_missing_bars(monkeypatch):
    import trader.manual_push as mp

    monkeypatch.setattr(mp, "_load_bars", lambda symbol: pd.DataFrame())

    message = mp.build_direction_review_message(["QQQ"], bias="偏多")
...
def test_manual_direction_review_message_scores_bias(monkeypatch):
    import trader.manual_push as mp

    monkeypatch.setattr(mp, "_load_bars", lambda symbol: _sample_intraday_bars())

    message = mp.build_direction_review_message(["SPY"], bias="偏多")

    assert "方向判断有效" in message.body
    assert message.fields["复盘方向"] == "偏多"
