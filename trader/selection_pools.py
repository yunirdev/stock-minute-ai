...
def build_weekly_focus_pool(
    ...
    *,
    ...
    if not symbols:
        symbols = build_source_universe(None)[: max(limit * 3, limit)]
    ...
    _progress(progress_callback, WEEKLY_FOCUS, 0, len(symbols), "scoring weekly focus pool")
    for idx, symbol in enumerate(symbols, start=1):
        quality_score, quality_reasons, quality_risks = _quality_score(symbol)
        setup = _feature_score(symbol, "1d", mode=WEEKLY_FOCUS)
        base_score = base_scores.get(symbol)
        ai_score = ai_scores.get(symbol)
        score, confidence, reasons, risks = _combine_weighted(
            parts={
                "base": (base_score, 0.25),
                "setup": ((setup or {}).get("score"), 0.50),
                "quality": (quality_score, 0.10),
                "ai": (ai_score, 0.15),
            },
            required_missing=[
                ("setup", setup is None, "缺少周级技术数据"),
            ],
        )
        reasons = quality_reasons + reasons + ((setup or {}).get("reasons", []))
        risks = quality_risks + risks + ((setup or {}).get("risks", []))
        if base_score is not None:
            reasons.append(f"长期池分 {base_score:.1f}")
        if ai_score is not None:
            reasons.append(f"AI 综合分 {ai_score:.1f}")
        status = _weekly_status(score, risks)
        items.append(PoolItem(
            symbol=symbol,
            rank=0,
            score=round(score, 1),
            ...
            layer=WEEKLY_FOCUS,
            component_scores={
                "base": _round_or_none(base_score),
                "setup": _round_or_none((setup or {}).get("score")),
                "quality": round(quality_score, 1),
                "ai": _round_or_none(ai_score),
            },
            reasons=_unique(reasons)[:6],
        ...
        if idx == len(symbols) or idx % 10 == 0:
            _progress(progress_callback, WEEKLY_FOCUS, idx, len(symbols), f"{symbol}: {status}")

    items.sort(key=lambda row: (_status_order(WEEKLY_FOCUS, row.status), -row.score, row.symbol))
    selected = items[: max(1, int(limit))]
    _rank(selected)
    if not symbols:
        warnings.append("周级池没有可用输入，已返回空结果")
    _progress(progress_callback, WEEKLY_FOCUS, len(symbols), len(symbols), f"selected {len(selected)}")
    return PoolResult(
        layer=WEEKLY_FOCUS,
        updated_at=now_s,
        source_size=len(symbols),
